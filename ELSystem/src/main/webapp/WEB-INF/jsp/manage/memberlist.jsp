...
<script type="text/javascript">
   ...
   function load(indexPage){
	   $.ajax({
		   type:'post',
		   url:'/manage/queryByPage/' + indexPage,
		   dataType:'JSON',
		   ...
		   success: function(data){
			   ...
			   if(data==null){
				   ...
				   table.append("<thead><tr><th>ID</th><th>姓名</th><th>帳號</th><th>Email</th><th>操作</th></tr></thead>");
				   
				   $.each(data, function(i,n){
					   if(n.mName == null){
						   n.mName=""
					   }
					   var tr = "<tbody><tr><td>"+n.memberID +"</td>"+
					   			"<td><a href='#'><img src='"+n.photo+"' class='rounded-circle' >" +  n.mName + "</a></td>" + 
					   			"<td>" +n.account + "</td>" +
					            "<td>" + n.email + "</td>" +"<td><a href='/manage/updateMember/" + n.memberID + 
					            "' class='settings'   ><i class='material-icons'>&#xE8B8;</i></a><a href='/manage/deleteMember/"+ n.memberID +
					            "' class='delete' onclick = dcheck()><i class='material-icons'>&#xE5C9;</i></a></td></tr></tbody>";
					   table.append(tr);
				   });		
    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            <div class="table-title" style="background-color:#FDC800">
                <div class="row">
                    <div class="col-sm-8">
                        <h2><b><font face="serif">會員管理</font></b></h2>
                    </div>
                    
                </div>
            </div>
            <div>
            	<form action="/manage/searchqueryByPage" method="post">
            		搜尋條件:
					<select name="searchoption">
						<option value="saccount">帳號</option>
					    <option value="semail">Email</option>
					</select>
					<input name="searchtext" type='text' style="height:19.6px;width:200px;">
					<input type='submit'>
				</form>						
            </div>
            <table id="showmember" class="table table-striped table-hover">
            	...
            	<nav aria-label="...">
				  <ul class="pagination pagination-sm">
				  	<c:forEach var="i" begin="1" end="${totalPages}" step="1">
				  		<li class="page-item">
				  		<button id="myPage" class="page-link" value="${i}" onclick="change(${i})">${i}</button>
				  		</li>			
					</c:forEach>
