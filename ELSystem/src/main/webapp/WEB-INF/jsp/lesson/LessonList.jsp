...
.sub-menu a:hover { color:#000000; background-color:#FFD306; text-decoration:none; }
	...
	<script type="text/javascript">
	   ...
	   function loadData(offset, limit, keywords) {
		   $.ajax({
			   type:'post',
			   url:'/lesson/searchByQuery',
			   data: JSON.stringify({
				   offset: offset || 0,
				   limit: limit || 10,
				   keywords: keywords || ''
			   ...
			   success: function(data){
				   ...
				   if(results===null || results.length === 0){
					  ...
					  // table.append("<thead><tr id='atitle'><th>ID</th><th>文章標題</th><th>分類</th></tr></thead>");
					  
					   
// 					   $.each(results, function(i,n){
//  						   var tr = "<tbody><tr align='left'>" + 
//  						            "<td>" + n.articleId + "</td>" +
//  						            "<td>" + n.title + "</td>" +
...
// 					   });
					   ...
					   var tr = $('#showarticle');
					   $.each(results, function(i,n){
						   var tr = "<div class='col-md-4 mb-4'>"+
						   			"<div class='card h-100'><img class='card-img-top w-100' height='200' src='" +n.image + "'  />"+
						   			"<div class='card-body'>"+
						   			"<h5 class='font-sans-serif fw-bold fs-md-0 fs-lg-1'>" + n.lessonName + "</h5><p>$"+ n.lessonPrice+"</p><button id='order' type='button' class='btn btn-primary btn-sm' value='order' onclick='sendCart(" + n.lessonId + ")'>加入購物車</button>"+
						   			"</div>"+
						   			"</div>"+
						   			"</div>"
					   ...
					   for(var i = 0; i< data.totalPages; i++) {
						   const pageBtn = '<button id="myPage" value=" '+ i +' " onclick="changePage('+i+')">' + (i + 1) + '</button>';
...
// 		   console.log($( "#keywords" ).val());
		   loadData(0, 10, keywords);
	   }
	   
	   function changePage(offset) {
		   const keywords= $( "#keywords" ).val();
		   loadData(offset, 10, keywords);
	   }
	   
	   
		
		function sendCart(lessonId){
			<%
			Object currentuserid = session.getAttribute("loginID");
			%>
			var loginUser  = "<%=currentuserid%>" ;
			var i = parseInt(loginUser,10);
			if(loginUser == 'null'){
				var params = {
						"lessonId":lessonId,
						"memberId":0
			...
			$.ajax({
				type:"post",
				url:"/shoppingcart/shoppingcartinsert.controller",
				dataType:'JSON',
				contentType:'application/json',
    ...
    <!-- <section> begin ============================-->
    	...
    	<div class="container-xl">
    		<div class="table-responsive" >
        		<div class="table-wrapper" style="background-color:#FDC800">
        	    	
            	    	<div class="row">
                	    	<div class="col-sm-8">
                    	    	<h2 id="articleListTitle"><b><font face="serif">課程總覽</font></b></h2>
                    		</div>
        ...
        <!-- end of .container-->
						...
						<table id="showpage">
						    <tr>
							  <td>總共 <span id="totalElements"></span> 筆 | 總頁數 <span id="totalPages"></span> |</td>
					...
					<table class="table table-striped table-hover" id="showarticle" ></table>
<!-- 					  <div class="clearfix"> -->
<!-- 					    <table id="showpage"> -->
<!-- 					    <tr> -->
<!-- 						  <td>總共 <span id="totalElements"></span> 筆 | 總頁數 <span id="totalPages"></span> |</td> -->
