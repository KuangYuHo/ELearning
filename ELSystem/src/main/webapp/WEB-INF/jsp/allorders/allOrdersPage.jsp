...
function load(indexPage){
	$.ajax({
		type:'post',
		url:'/orders/findallorders/'+ indexPage,
		dataType:'JSON',
		...
		success: function(data){
			...
			if(data==null){
				...
				var table = $('#showorders');
				table.append("<thead><tr align='center'><th>訂單編號</th><th>會員編號</th><th>付款金額</th><th>付款日期</th><th></th></tr></thead>");
				
				$.each(data, function(i,n){
					var tr = "<tbody><tr align='center'>" + "<td>" + "<a href='findbyorderid/" + n.orderId + "'>" + n.orderId + "</a>" + "</td>" +
							 "<td>" + n.memberId + "</td>" +
							 "<td>" + n.payPrice + "</td>" +
							 "<td>" + n.payDate + "</td>" + 
							 "<td><a href='/orders/delete/" + n.orderId + "' onclick='recheck()'>刪除</a></td>" +
							 "</tr></tbody>";
					table.append(tr);
				});
    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            <div class="table-title" style="background-color:#FDC800">
                <div class="row">
                    <div class="col-sm-12">
                        <h2><b><font face="serif">訂單總表</font></b></h2>
                    </div>
                    
                </div>
            </div>
            <table id="showorders" class="table table-striped table-hover">
                
            </table>
            <div class="clearfix">
                <table id="showpage">
					<tr>
						<td>Total Pages:${odTotalPages } Total Records:${odTotalElements }</td>
						<td colspan="3" align="right">Previous
							<c:forEach var="i" begin="1" end="${odTotalPages }" step="1">
