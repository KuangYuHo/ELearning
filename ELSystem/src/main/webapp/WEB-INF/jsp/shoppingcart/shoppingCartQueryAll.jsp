...
<script type="text/javascript">
	...
	function load(indexPage){
		$.ajax({
			type:'post',
			url:'/shoppingcart/queryByPage/'+ indexPage,
			dataType:'JSON',
			...
			success: function(data){
				...
				if(data==null){
					...
					var table = $('#showcart');
					table.append("<thead><tr align='center'><th>課程圖片</th><th>課程編號</th><th>課程名稱</th><th>課程價格</th><th><a href='/shoppingcart/deleteall'>清除</a></th></tr></thead>");
					
					$.each(data, function(i,n){
						var tr = "<tbody><tr align='center'>" + "<td><img  width='150' src='" + n.lesson.image + "'></td>" + "<td>" + n.lessonId + "</td>" +
								 "<td>" + n.lesson.lessonName + "</td>" + "<td>" + n.lesson.lessonPrice + "</td>" +
								 "<td><a href='/shoppingcart/delete/" + n.shoppingCartId + "'onclick='recheck()'>刪除</a></td></tbody>" +
								 "</tr>";
						table.append(tr);
					});
					...
					var total = 0;
					$.each(data, function(i,n){
						total += n.lesson.lessonPrice
					});
					if(total == 0){
						table.append("<tfoot><tr align='center'><td colspan='4'>總計 : " + total + "</td><td><form action='/orders/payPage' method='GET'><input type='submit' class='btn btn-warning' value='全部結帳' disabled='true'></form></td></tr></tfoot>");
					}else{
						table.append("<tfoot><tr align='center'><td colspan='4'>總計 : " + total + "</td><td><form action='/orders/payPage' method='GET'><input type='submit' class='btn btn-warning' value='全部結帳'></form></td></tr></tfoot>");
					}
    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            <div class="table-title" style="background-color:#FDC800">
                <div class="row">
                    <div class="col-sm-12">
                        <h2><b><font face="serif">購物車</font></b></h2>
                    </div>
                    
                </div>
            </div>
            <table id="showcart" class="table table-striped table-hover">
                
            </table>
            <div class="clearfix">
                <table id="showpage">
					<tr>
						<td>Total Pages:${sTotalPages } Total Records:${sTotalElements }</td>
						<td colspan="3" align="right">Previous
							<c:forEach var="i" begin="1" end="${sTotalPages }" step="1">
