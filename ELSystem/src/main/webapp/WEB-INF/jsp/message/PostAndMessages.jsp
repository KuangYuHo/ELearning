...
<!--                 </select> -->
            ...
            <div class="inner-main-body p-2 p-sm-3 collapse forum-content show">
            	<div class="card mb-2">
                	<h2 style="text-align:center;">管理該貼文的留言</h2>
            	</div>   
            </div>   
            <div class="inner-main-body p-2 p-sm-3 collapse forum-content show">
                <div class="card mb-2">
               <form id="deleteaction" action="deletemessage"  method="get">
	<table border="1">
		<tr>			
			<th ></th>
			<th  colspan="3">${addPost.postTitle}</th>
		</tr>
		<tr>			
			<td >時間:</td>
			<td colspan="3">${addPost.postDate}</td>
		</tr>
		<tr>			
			<td >內容:</td>
			<td colspan="3">${addPost.postInformation}</td>
		</tr>

		<c:forEach var="message"   items="${addPost.messages}" >
			<tr>
				<td>留言:	<input type='hidden' name = 'messageId' value='${message.messageId}'></td>
				<td align="center">${message.messageinformation}</td>
<%-- 				<td>${message.messageDate}</td> --%>
				<td align="center"><input  type='submit' value='刪除' name=${message.messageId} onclick="return confirmDelete('${message.messageId}');" >
...
<script type="text/javascript">
  function confirmDelete(messageId){
	  var result = confirm("確定刪除此筆記錄(帳號:" + messageId.trim() + ")?");
	  if (result) {
  ...
  function confirmUpdate(messageId){
	  var result = confirm("確定送出此筆記錄(帳號:" + messageId.trim() + ")?");
	  if (result) {
// 		  document.forms[0].putOrDelete.name = "_method";
